...
import { ThemeType, ViewType, THEMES, ThemeConfig, ProxyConfig, GameMessage, SaveFile, StatusData } from '../types';
...
interface GameState {
  ...
  updateCurrentStreamStats: (stats: Partial<StreamStats> | ((prev: StreamStats | null) => StreamStats)) => void;
  resetStreamStats: () => void;
  setSystemLogs: (log: string | ((prev: string) => string)) => void;
  saves: SaveFile[];
  ...
  worldCreation: {
    ...
    mcData: {
      ...
      innerSecret: string;
      relationships: Array<{ name: string; relation: string; status?: string; description: string }>;
      loveViews: string;
      experience: string;
  ...
  updateWorldCreation: (data: Partial<GameState['worldCreation']> | ((state: GameState['worldCreation']) => void)) => void;
...
const customIdbStorage: PersistStorage<GameState> = {
  getItem: async (name: string): Promise<StorageValue<GameState> | null> => {
    return (await storageService.loadItem<StorageValue<GameState>>(name)) || null;
  },
  setItem: async (name: string, value: StorageValue<GameState>): Promise<void> => {
...
export const useStore = create<GameState>()(
  persist(
    immer((set) => ({
      ...
      updateCurrentStreamStats: (stats) => set((state) => {
        if (typeof stats === 'function') {
          ...
          state.currentStreamStats = { ...state.currentStreamStats, ...stats };
        }
      }),
      resetStreamStats: () => set((state) => {
      ...
      setSelectedAIModel: (model) => set((state) => {
        state.selectedAIModel = model;
      }),
      setPlayerRules: (rules) => set((state) => {
      ...
      addProxy: (proxy) => set((state) => {
        state.proxies = [...state.proxies.filter(p => p.id !== proxy.id), proxy];
        if (!state.activeProxyId) state.activeProxyId = proxy.id;
      }),
      updateProxy: (id, newProxy) => set((state) => {
      ...
      setActiveProxy: (id) => set((state) => {
        state.activeProxyId = id;
      }),
      setGlobalProxyEnabled: (enabled) => set((state) => {
        state.globalProxyEnabled = enabled;
      }),
      addPersonalApiKey: (key) => set((state) => {
        if (!state.personalApiKeys.includes(key)) {
          state.personalApiKeys.push(key);
        }
      }),
      removePersonalApiKey: (key) => set((state) => {
        state.personalApiKeys = state.personalApiKeys.filter(k => k !== key);
      }),
      setTargetWordCount: (count) => set((state) => {
      ...
      setMemoryFullTurnsCount: (count) => set((state) => {
        state.memoryFullTurnsCount = count;
      }),
      setMemoryLogsCount: (count) => set((state) => {
        state.memoryLogsCount = count;
      }),
      setFullScreenStream: (active) => set((state) => {
        state.isFullScreenStream = active;
      }),
      setIsGeneratingStream: (active: boolean) => set((state) => {
        state.isGeneratingStream = active;
        if (active) {
          state.streamStartTime = Date.now();
        }
      }),
      updateStreamData: (data) => set((state) => {
        state.fullScreenStreamData = typeof data === 'function' ? data(state.fullScreenStreamData) : data;
      }),
      setSystemLogs: (log) => set((state) => {
        state.systemLogs = typeof log === 'function' ? log(state.systemLogs) : log;
      }),
      updateWorldCreation: (data) => set((state) => {
      ...
      saveCurrentGame: async () => {
        ...
        const getExactTurnCount = (msgs: GameMessage[]) => {
          const aiMsgsCount = msgs.filter(m => m.sender === 'ai' || m.sender === 'system').length;
        ...
        set((draft) => {
          ...
          const existingIdx = draft.saves.findIndex(s => {
            ...
            return s.gameData?.id === draft.gameData.id && 
                   !s.id.endsWith('_auto') &&
      ...
      loadSave: async (id) => {
        ...
        set((draft) => {
          ...
          if (s) {
            ...
            const lastAiMsg = [...s.messages].reverse().find(m => m.sender === 'ai' || m.sender === 'system');
      ...
      clearSaves: () => set((state) => {
        state.saves = [];
      }),
      importSaves: (newSaves) => set((state) => {
        if (Array.isArray(newSaves)) {
          const validSaves = newSaves.filter(s => s.id && s.name && s.gameData);
          validSaves.forEach(newSave => {
            // Khi tải từ máy lên, nếu tên lưu y hệt thì lưu đè
            const existingIdx = state.saves.findIndex(s => s.name === newSave.name);
            if (existingIdx >= 0) {
      ...
      resumeLatestGame: async () => {
        ...
        if (state.saves.length > 0) {
          const latest = [...state.saves].sort((a, b) => b.updatedAt - a.updatedAt)[0];
          if (latest.ragMemories && latest.gameData && latest.gameData.id) {
             await ragService.setMemories(latest.gameData.id, latest.ragMemories);
          }
          set((draft) => {
            draft.gameData = latest.gameData;
            draft.messages = latest.messages;
            const lastAiMsg = [...latest.messages].reverse().find(m => m.sender === 'ai' || m.sender === 'system');
            if (lastAiMsg && lastAiMsg.fullStreamLog) {
              draft.fullScreenStreamData = lastAiMsg.fullStreamLog;
            } else if (lastAiMsg && lastAiMsg.thought) {
              draft.fullScreenStreamData = lastAiMsg.thought;
    ...
    })),
    {
      ...
      merge: (persistedState: any, currentState: GameState) => {
        ...
        if (persistedState.theme?.id) {
          merged.theme = THEMES.find(t => t.id === persistedState.theme.id) || currentState.theme;
        }
        return merged;
      },
    }
