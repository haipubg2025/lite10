    ...
    try {
      ...
      if (parsedData) {
        ...
        const sanitizeStatusData = (statusObj: any) => {
          ...
          const cleanObj: any = {};
          ['psychological', 'physiological', 'health', 'condition'].forEach((key: string) => {
            ...
            parsedData.npcUpdates.forEach((upd: any) => {
              if (upd.id && upd.updates && typeof upd.updates === "object") {
                ...
                if (idx !== -1) {
                  ...
                  if (cNpc.statusData) {
                    cNpc.statusData = sanitizeStatusData(cNpc.statusData);
                  }
        ...
        prev.map((msg) =>
          msg.id === aiMsgId
            ? {
                ...
                isStreaming: false,
                content:
                  "Matrix Lite v3 bị nhiễu loạn băng thông, quá trình tính toán bị ngắt quãng. Vui lòng thử lại.\nLỗi hệ thống: " +
                  errorMsg,
              }
            : msg,
        ),
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <AnimatePresence>
          {leftOpen && (
            ...
            >
              <div
                className={`p-4 border-b shrink-0 ${theme.group === "Dark" ? "border-white/10" : "border-black/10"}`}
              >
                <h3
                  className={
                  ...
                  .map(({ npc, index, locStr, isUnknownLoc, isMale }) => {
                    ...
                    return (
                      <div
                        ...
                        className={`p-4 rounded-2xl border transition-all cursor-pointer ${bgClass} ${borderClass} ${opacityClass}`}
                      >
                        ...
                        <>
                            ...
                            (turn.aiMsg.mainText || turn.aiMsg.content) && (
                              ...
                              >
                                <Markdown remarkPlugins={[remarkGfm]}>
                                  {(turn.aiMsg.mainText || turn.aiMsg.content || '').replace(/<br\s*\/?>/gi, '\n')}
                                </Markdown>
                              </div>
                            )
                          )}
                          ...
                          {turn.aiMsg.suggestedActions &&
                            turn.aiMsg.suggestedActions.length > 0 && (
                              ...
                              >
                                <h4
                                  className={`text-sm font-bold uppercase tracking-widest mb-4 opacity-80 ${theme.group === "Dark" ? "text-blue-400" : "text-slate-700"}`}
                                >
        ...
        <AnimatePresence>
          {rightOpen && (
            ...
            >
              ...
              >
                <div
                  className={`font-mono text-[11px] leading-relaxed break-words whitespace-pre-wrap ${theme.group === "Dark" ? "text-green-400/80" : "text-[#0f172a] font-medium"}`}
                >
                  {fullScreenStreamData ||
              ...
              <div
                className={`h-48 shrink-0 p-4 overflow-y-auto custom-scrollbar scroll-smooth ${theme.group === "Dark" ? "bg-red-950/20" : "bg-red-50/40 border border-red-100 rounded-xl m-2 shadow-inner"}`}
              >
                <div
                  ...
                  <div className="w-full space-y-8 py-4 px-4 md:px-8">
                    ...
                    <div className="p-5 rounded-2xl bg-gradient-to-br from-purple-500/10 to-indigo-500/10 border border-purple-500/20">
                      <p className="text-slate-700 dark-theme:text-white/80 text-sm leading-relaxed">
                        ...
                        . Game Matrix Lite v3 sử dụng hệ thống RAG
        ...
        {expandedLog && (
          ...
          >
            <motion.div
              ...
              className={`w-full max-w-5xl h-[80vh] rounded-xl flex flex-col overflow-hidden shadow-2xl ${theme.group === "Dark" ? "theme-panel text-white" : theme.bgClass} ${theme.group === 'Dark' ? 'border border-white/10' : 'border border-black/10'}`}
              onClick={(e) => e.stopPropagation()}
            >
              ...
              <div
                className={`flex-1 p-6 overflow-y-auto custom-scrollbar ${expandedLog === "error" ? "bg-red-950/20" : ""} ${theme.group === "Dark" ? "theme-panel !border-none" : theme.bgClass}`}
              >
                <div
