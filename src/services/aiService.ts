...
class AIService {
  ...
  public async *generateStreamingContent(prompt: string, schema?: any, systemInstruction?: string) {
    ...
    while (true) {
      ...
      try {
        const rawStream = this.generateStreamingContentRaw(prompt, schema, systemInstruction, providedApiKey);
   ...
   */
  private async *generateStreamingContentRaw(prompt: string, schema?: any, systemInstruction?: string, providedApiKey?: string | null) {
    let attempt = 0;
    while (true) {
      attempt++;
      const state = useStore.getState();
